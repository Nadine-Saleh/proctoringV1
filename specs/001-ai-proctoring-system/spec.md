...
- Q: What is the authoritative trigger for `face_too_close` / `face_too_far` violations? → A: Per-student calibrated baseline only. The pre-exam distance-calibration step captures an `optimal_distance_cm` per session; `face_too_close` and `face_too_far` fire when the live estimated distance exceeds the configured tolerance band around that baseline. Hardcoded global `faceRatio` thresholds are removed from the detection loop — calibration becomes load-bearing for these two violation types.
- Q: When distance calibration fails or is skipped, what should happen? → A: Soft fallback — the session proceeds using a conservative server-side default baseline (50 cm ± 20 cm) and is flagged with `calibration_skipped = true` so the instructor can weight downstream `face_too_close` / `face_too_far` events accordingly during review.
- Q: What severity should `face_too_close` and `face_too_far` carry in the canonical taxonomy? → A: `low` (severity 5) for both. Distance drift is the most ambiguous signal in the distance category (often posture/seat shift, not intent) and must not dominate the cheating score on noisier `calibration_skipped = true` sessions.
- Q: Should the calibrated baseline persist server-side per session? → A: Yes — persisted on `exam_sessions` as `optimal_distance_cm`, `distance_tolerance_cm`, and `calibration_skipped`, written once at `start_exam_session`. Required so instructors can audit "what baseline produced these distance violations" during evidence review (FR-028) and so distance violations remain interpretable post-exam.
...
- **Reference embedding missing**: A student has no stored reference embedding (first-ever exam). The system routes them to an inline reference-capture screen (within the join flow) before allowing any verification attempt to count against their retry budget. Capture quality is enforced by requiring three on-demand frames whose pairwise embedding similarity passes a configured threshold (see FR-009 / `useReferenceCapture`).
...
- **Instructor closes the exam after publication**: Once the exam is closed (via `close_exam`), no new joins are accepted; students with active sessions are allowed to finish unless the instructor explicitly terminates them.
...
- **Very short exams**: An exam whose duration is shorter than the combined verification + minimum monitoring warm-up time MUST either be rejected at publication or the verification step MUST not consume exam time. Resolved by FR-002a: the exam clock starts only after `start_exam_session` is called (post-verification), so verification time is excluded from the student's exam duration.
- **Distance calibration cannot complete**: The student's camera is unavailable, no face is detected, or the estimated distance never stabilizes within the calibration tolerance. Resolved by FR-013b: the session proceeds with the server-side default baseline (50 cm ± 20 cm) and `calibration_skipped = true`; instructor surfaces flag the session so distance-derived events can be discounted during review.
...
- **FR-002**: The system MUST validate that an exam has at least one question and a duration of at least **5 minutes** before allowing publication. Publication MUST be rejected with a specific error if either guard fails.
- **FR-002a**: The exam clock (used for window-close auto-submit and student-visible time remaining) MUST start when the student calls `start_exam_session` (i.e., after identity verification has passed). Time spent in `awaiting_verification` and `verified` states MUST NOT be deducted from the student's exam duration. This ensures verification overhead can never consume exam time, satisfying the "very short exam" edge case without requiring an arbitrary minimum-duration calculation.
...
- **FR-013**: During an active exam session, the system MUST continuously evaluate the student's behavior across at least these categories: gaze direction, face presence, additional persons in frame, tab/window focus loss, camera availability, and face-to-camera distance.
- **FR-013a**: Before the exam clock starts (per FR-002a), the student MUST complete a one-time per-session distance-calibration step that captures their `optimal_distance_cm` baseline. The session MUST persist this baseline plus a configurable tolerance band (`distance_tolerance_cm`, default ±15 cm) on `exam_sessions`, written once at `start_exam_session` and immutable thereafter for that session. During the exam, the detection loop MUST emit `face_too_close` when the live estimated distance is below `optimal_distance_cm − distance_tolerance_cm` and `face_too_far` when it exceeds `optimal_distance_cm + distance_tolerance_cm`. No hardcoded global distance threshold may run in parallel — the per-student calibrated baseline is the sole authoritative trigger for these two violation types. The persisted baseline MUST be included in the instructor evidence package (FR-025) so distance violations remain auditable post-exam.
- **FR-013b**: If distance calibration cannot complete (camera unavailable, face not detected, or persistent instability), the session MUST proceed with a server-side conservative default baseline of **50 cm ± 20 cm** and MUST set `exam_sessions.calibration_skipped = true`. The detection loop MUST continue to emit `face_too_close` / `face_too_far` against this fallback band. Instructor-facing surfaces (oversight dashboard, evidence package) MUST visibly mark sessions where `calibration_skipped = true` so reviewers can appropriately discount distance-derived events.
- **FR-013c**: `face_too_close` and `face_too_far` MUST be registered in the canonical violation taxonomy with severity `low` (numeric weight = 5). They MUST be visible to the instructor and contribute to the live cheating score, but their per-event weight MUST NOT exceed the weight of `gaze_off_screen` or higher-confidence categories — distance drift is the most ambiguous signal in the distance category and must not dominate the score, especially on `calibration_skipped = true` sessions.
...
- **Exam Session**: The live instance of a student taking an exam. Attributes: student, exam, admit timestamp, current status (`awaiting_verification`, `verified`, `in_progress`, `submitted`, `auto_submitted`, `terminated`), live cheating score, `optimal_distance_cm` (numeric, persisted; the calibrated baseline per FR-013a), `distance_tolerance_cm` (numeric, persisted; default 15), and `calibration_skipped` (boolean, true when the conservative default per FR-013b was used). The three calibration fields are written once at `start_exam_session` and are immutable thereafter for that session. Only `in_progress` sessions are eligible to emit new violation events or score updates.
- **Violation Event**: A single detected integrity deviation. Attributes: session, type (gaze, face-presence, multiple-persons, focus-loss, camera-loss, face-too-close, face-too-far, etc.), timestamp, severity (`low` | `medium` | `high` | `critical`), optional evidence snippet reference.
