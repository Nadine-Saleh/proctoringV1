...
interface UseExamSessionReturn {
  ...
  joinExam: (accessCode: string) => Promise<boolean>;
  verifyIdentity: (
...
export function useExamSession(): UseExamSessionReturn {
  ...
  const [lifecycleStatus, setLifecycleStatus] =
    useState<SessionLifecycleStatus>('idle');
  const [joinData, setJoinData] = useState<JoinExamResponse | null>(null);
  const [questions, setQuestions] =
    useState<StartSessionResponse['questions'] | null>(null);
  ...
  const startHeartbeat = useCallback((sessionIdArg: string) => {
    ...
    const heartbeat = new SessionHeartbeat(30000);
  ...
  // ── Hydrate session from URL sessionId ─────────────────────────────────────
  ...
  const joinExam = useCallback(async (accessCode: string): Promise<boolean> => {
    setIsLoading(true);
    setError(null);

    try {
      const result = await IdentityVerificationService.joinExam(accessCode);
  ...
  const startSession = useCallback(
    async (
      ...
      try {
        ...
        if (!supabaseUser) {
          const errorMsg =
            'User not authenticated with Supabase. Please sign in again.';
        ...
        if (supabaseUser.id !== studentId) {
          console.warn(
            '[useExamSession] User ID mismatch: auth=',
            supabaseUser.id,
            'provided=',
            studentId
          );
        ...
        const existingResult = await ExamSessionService.getActiveSession(
          studentId,
          examId
        );
        ...
        if (existingResult.success && existingResult.session) {
          console.log(
            '[useExamSession] Found existing active session:',
            existingResult.session.id
          );
        ...
        if (!result.success || !result.session) {
          console.error(
            '[useExamSession] Failed to create session:',
  ...
  const submitExam = useCallback(
    async (
      ...
      try {
        ...
        if (result.success) {
          const persisted = await ExamSessionService.submit(
            targetSessionId,
            durationSeconds
          );
          ...
          if (!persisted.success) {
            console.warn(
              '[useExamSession] Fallback session.submit failed:',
              persisted.error
            );
          }
